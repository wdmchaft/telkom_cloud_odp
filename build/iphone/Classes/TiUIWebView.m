...
static NSString * const kTelkom_CloudJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
-(NSString*)_telkom_cloudInjection
{
	...
	[html appendString:kTelkom_CloudJavascript];
...
{
	// attempt to make well-formed HTML and inject in our Telkom_Cloud bridge code
	...
	if (range.location!=NSNotFound)
	{
		...
		if (nextRange.location!=NSNotFound)
		{
			...
			[html appendString:[self _telkom_cloudInjection]];
	...
	else
	{
		...
		if ([[NSFileManager defaultManager] fileExistsAtPath:path])
		{
			...
			else
			{
				...
				{
					NSLog(@"[WARN] I have no idea what the appropriate text encoding is for: %@. Please report this to Telkom_Cloud support.",url);
				}
			}
